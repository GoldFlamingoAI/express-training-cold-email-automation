# ⚠️ STOP — REVIEW REQUIRED BEFORE ANY EDITS

This file is the Codex workflow version of `CLAUDE.md`. The existing `CLAUDE.md`
in this repo has project-specific content that must be preserved.

**Do NOT edit `CLAUDE.md` yet.**

**What you must do first:**
1. Read this file in full
2. Read the existing `CLAUDE.md` at the repo root in full
3. Produce a side-by-side analysis showing:
   - What sections exist in this file but not in `CLAUDE.md`
   - What sections exist in `CLAUDE.md` that would be affected by a merge
   - Any conflicts or overlaps between the two
   - A recommended merge strategy with exactly what to prepend, what to keep, and what to discard
4. Present the analysis to the user and wait for explicit approval before touching `CLAUDE.md`

Do not merge, edit, replace, or delete anything until the user reviews the analysis and says "go ahead."
...
Loaded:
  AGENTS.md ✓  PHASES.md ✓  CODEX_TASK_RECIPES.md ✓  NOTES.md ✓  ERRORS.md ✓
...
| `CODEX_TASK_RECIPES.md` | session start | session start + calibration edits | Claude only |
...
Ready. Available commands:
  /codex-brief    — minimal output mode
  /codex-phase    — current phase and task progress
  /codex-overview — architecture and build state at a glance
  /codex-doc      — generate a .md doc for any component
  /codex-errors   — surface errors, deviations, and BLOCKED signals
  /codex-notes    — review all NOTES.md entries
  /codex-emergency — check for active emergencies
  /codex-status   — full Codex workflow state at a glance
  /codex-skills   — show this list again
...
- Codex reads `AGENTS.md`, `PHASES.md`, `CODEX_TASK_RECIPES.md`, `NOTES.md`, `ERRORS.md` at session start
...
4. If 1+ patterns hitting threshold: tighten AGENTS.md / CODEX_TASK_RECIPES.md directly on GitHub, log entry in PHASES.md "Codex Calibration Log"
...
2. Update CODEX_TASK_RECIPES.md — strip Phase [X] recipes, load Phase [X+1] recipes
3. Update `docs/codex/TRACKING.md` with Phase [X] final verdict
4. Commit all three files directly
5. Tell the user:
> Phase [X] audit complete — all clear. CODEX_TASK_RECIPES.md updated for Phase [X+1].
...
Any time you update AGENTS.md, CODEX_TASK_RECIPES.md, CLAUDE.md, or any file Codex reads at
...
3. **Tell the user explicitly:**
   ...
   - That they need to bring Claude in to fix AGENTS.md and/or CODEX_TASK_RECIPES.md
...
Once AGENTS.md/CODEX_TASK_RECIPES.md are fixed and the bad code is corrected, delete
...
Calibration   : [any AGENTS.md / CODEX_TASK_RECIPES.md tightening done this session]
