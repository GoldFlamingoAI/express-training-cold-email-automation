...
Use the mini sections below when creating row 1. The **Column titles** row matches the screenshot style; the **Copy/paste row** is tab-separated so you can still paste it directly into cell `A1`.

### SETTINGS

| What to use | Value |
|---|---|
| Column titles | `key` , `value` |
| Copy/paste row | `key	value` |

### COMPANIES

| What to use | Value |
|---|---|
| Column titles | `company` , `website` , `industry` , `city` , `state` , `employeeSize` , `sourceUrl` , `wtfpRelevance` |
| Copy/paste row | `company	website	industry	city	state	employeeSize	sourceUrl	wtfpRelevance` |

### CONTACTS

| What to use | Value |
|---|---|
| Column titles | `contactId` , `email` , `firstName` , `company` , `title` , `linkedin` , `status` , `verificationResult` , `catchAll` , `maConfirmed` , `roleIsRelevant` , `employeeSizeFit` , `industryFit` , `wtfpRelevance` , `personalizationLine` , `emailsSent` , `lastSentAt` , `isSuppressed` |
| Copy/paste row | `contactId	email	firstName	company	title	linkedin	status	verificationResult	catchAll	maConfirmed	roleIsRelevant	employeeSizeFit	industryFit	wtfpRelevance	personalizationLine	emailsSent	lastSentAt	isSuppressed` |
...
| What to use | Value |
|---|---|
| Column titles | `timestamp` , `email` , `reason` , `source` |
| Copy/paste row | `timestamp	email	reason	source` |

### ACTIVITY_LOG

| What to use | Value |
|---|---|
| Column titles | `timestamp` , `stage` , `action` , `contactId` , `details` , `status` |
| Copy/paste row | `timestamp	stage	action	contactId	details	status` |

### TEMPLATES

| What to use | Value |
|---|---|
| Column titles | `subject` , `body` |
| Copy/paste row | `subject	body` |

### DASHBOARD

| What to use | Value |
|---|---|
| Column titles | Leave row 1 blank. DashboardService writes here and populates the tab. |
| Copy/paste row | Nothing to paste. |

### CAMPAIGNS

| What to use | Value |
|---|---|
| Column titles | Reserved tab; headers optional for MVP. |
| Copy/paste row | Nothing required for MVP. |
