...
function runDraftPipeline() {
  try {
    ...
    contacts.forEach(function(contact) {
      ...
      if (settings.draftOnly === false) {
        auditLog('Orchestrator', 'DRAFT_ONLY_OVERRIDE', contact.contactId || '', 'DRAFT_ONLY is false; creating draft only', 'WARN');
      }

      const draftResult = createDraft(contact.email, template.subject, body, contact.contactId || '');
