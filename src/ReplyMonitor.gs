...
function monitorReplies() {
  ...
  try {
    ...
    for (let index = 0; index < table.rows.length; index += 1) {
      ...
      if (threads.length === 0) {
        auditLog(REPLY_MONITOR_STAGE, 'reply_not_detected', contactId, { email: email }, 'OK');
        continue;
      }
      ...
      summary.updated += 1;
      auditLog(REPLY_MONITOR_STAGE, 'reply_detected', contactId, {
        email: email,
        threadsFound: threads.length,
      }, 'OK');
    }

    auditLog(REPLY_MONITOR_STAGE, 'monitor_complete', '', summary, 'OK');
    return summary;
  } catch (error) {
    auditLog(REPLY_MONITOR_STAGE, 'monitor_failed', '', {
