...
const ZERO_BOUNCE_CLIENT_DEFAULT_CREDITS_URL = 'https://api.zerobounce.net/v2/getcredits';
const ZERO_BOUNCE_CLIENT_SETTING_VALIDATE_URL = 'ZEROBOUNCE_VALIDATE_URL';
const ZERO_BOUNCE_CLIENT_SETTING_CREDITS_URL = 'ZEROBOUNCE_CREDITS_URL';
...
/**
 * Reads the current ZeroBounce credit balance.
 * @returns {{success: boolean, credits: number|null, raw: Object|null, error: string|null}}
 */
function getZeroBounceCredits() {
  try {
    const settings = getZeroBounceClientSettings_();
    const apiKey = getZeroBounceClientApiKey_();

    // NEEDS_WIFI_TEST: UrlFetchApp.fetch calls the live ZeroBounce credits API.
    const response = UrlFetchApp.fetch(buildZeroBounceClientUrl_(settings.creditsUrl, { api_key: apiKey }), {
      method: 'get',
      muteHttpExceptions: true,
    });
    const result = parseZeroBounceClientResponse_(response);
    ...
    if (result.error) {
      auditLog(ZERO_BOUNCE_CLIENT_STAGE, 'credits_failed', '', JSON.stringify({ error: result.error }), 'ERROR');
      return { success: false, credits: null, raw: result, error: result.error };
    }

    auditLog(ZERO_BOUNCE_CLIENT_STAGE, 'credits_complete', '', JSON.stringify({ credits: Number(result.Credits) }), 'OK');
    return {
      success: true,
    ...
    const message = error && error.message ? error.message : String(error);
    auditLog(ZERO_BOUNCE_CLIENT_STAGE, 'credits_error', '', JSON.stringify({ message: message }), 'ERROR');
    return { success: false, credits: null, raw: null, error: message };
  }
}
...
function getZeroBounceClientSettings_() {
  ...
  return {
    ...
    creditsUrl: String(settings[ZERO_BOUNCE_CLIENT_SETTING_CREDITS_URL] || ZERO_BOUNCE_CLIENT_DEFAULT_CREDITS_URL).trim(),
